...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_RELOAD) then
        -- Save current ADS progress so reload completion
        -- does not count as a new ADS input.
...
function ADS_states.normal.update(this, context)
    local aimingProgress = context:getAimingProgress()
    local mainTrack = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)

    -- MAIN_TRACK is still busy (reload, bolt, melee, etc.)
    if (not context:isStopped(mainTrack)) then
        this.ADS_states.aiming_progress = aimingProgress
        return
    end

    -- Only trigger aim when ADS progress actually increases.
    -- This prevents reload completion from restarting aim_start.
    if (aimingProgress > this.ADS_states.aiming_progress) then
