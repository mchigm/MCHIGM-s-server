...
local default = require("daffas_arsenal_default_daffas_state_machine")
...
function normal_state.update(this, context)
    ...
    if (isNoAmmo(context)) then
        ...
        if (a < 11) then
            context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK),0.1+(10-a)*0.5,false)
        else
